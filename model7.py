...
# Adding Registration Date Lead Lag
#-----------------------------------
data["Registration_Date_lead"] = data.groupby("Patient_ID")["Registration_Date"].shift(1)
data["Registration_Date_lag"] = data.groupby("Patient_ID")["Registration_Date"].shift(-11)
date_columns += ["Registration_Date_lead","Registration_Date_lag"]
...
data["camp_time"] = (data["CD_Camp_End_Date"] - data["CD_Camp_Start_Date"])/np.timedelta64(1, 'D')
...
# Lead and Lags of Variable
#--------------------------------------------------------
data["target_lead"] = data.sort_values(by="Registration_Date").groupby("Patient_ID")["target"].shift(1)
data["target_lag"] = data.sort_values(by="Registration_Date").groupby("Patient_ID")["target"].shift(-1)

extra_features = []
extra_features += ["time_since_interaction","time_since_camp_start","time_before_camp_ends","camp_time"]
